...
const POSTS_PER_ROUND = 8;      // how many copies of a message go out per batch
const DELETE_AFTER_MS = 8_000;  // how long the batch sits after the last one is sent
const SEND_GAP_MS = 1_000;      // gap between each of the 8 sends (~7s total to send all 8)
...
async function postBatch(text) {
  // Send all 8 copies with a small gap between each
  const sentMessages = [];
