...
function nextAutoMessage() {
  if (autoMessages.length === 0) return null;
...
async function runRound() {
  ...
  if (text === null) {
    console.log('LIVE message pool is empty — nothing to post. Add one with /addmsg.');
    await wait(5000);
    if (isLive) runRound();
    return;
  }
...
const AUTO_INTERVAL_FILE = path.join(__dirname, 'auto-interval.json');
const DEFAULT_AUTOPOST_MINUTES = 10;

function loadAutopostMinutes() {
  try {
    const raw = fs.readFileSync(AUTO_INTERVAL_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (parsed.minutes && parsed.minutes > 0) return parsed.minutes;
  } catch (err) {
    // no file yet — use default
  }
  return DEFAULT_AUTOPOST_MINUTES;
}
function saveAutopostMinutes(minutes) {
  fs.writeFileSync(AUTO_INTERVAL_FILE, JSON.stringify({ minutes }));
}

let autopostMinutes = loadAutopostMinutes();
let autopostTimer = null;

function startAutopostTimer() {
  if (autopostTimer) clearInterval(autopostTimer);
  autopostTimer = setInterval(autoPostOnce, autopostMinutes * 60 * 1000);
}

async function autoPostOnce() {
  const message = nextAutoMessage();
  if (message === null) {
    console.log('AUTO-POST message pool is empty — nothing to post. Add one with /addauto.');
    return;
  }
  const isPhoto = typeof message === 'object' && message.photo;
  try {
...
// Change how often the 24/7 auto-post fires, e.g. /setinterval 10
bot.onText(/\/setinterval (\d+)/, (msg, match) => {
  if (String(msg.from.id) !== String(adminId)) return;
  const minutes = parseInt(match[1], 10);
  if (minutes < 1) {
    bot.sendMessage(msg.chat.id, 'Please send a number of minutes greater than 0.');
    return;
  }
  autopostMinutes = minutes;
  saveAutopostMinutes(minutes);
  startAutopostTimer();
  bot.sendMessage(msg.chat.id, `⏱ Auto-post interval set to every ${minutes} minute(s).`);
});

// Runs 24/7, independent of /live start|stop.
startAutopostTimer();
